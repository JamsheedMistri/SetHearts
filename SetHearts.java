							...
							try {
								if (Double.parseDouble(args[1]) <= 1024) {
									for(Player victim : Bukkit.getOnlinePlayers()) {
										victim.setHealthScale(Double.parseDouble(args[1]) * 2);
										victim.setMaxHealth(Double.parseDouble(args[1]) * 2);
										victim.setHealth(Double.parseDouble(args[1]) * 2);
										p.sendMessage(prefix + "Set the amount of hearts of everyone to " + heart + (Double.parseDouble(args[1])));
									}
								}
